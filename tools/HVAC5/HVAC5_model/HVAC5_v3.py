...
###################################
# функция анализа принятого запроса
###################################
...
#################################################
# Функция для шапки логирования HVAC5
#################################################
def log_system_state_Head_HVAC5():
    # Заголовок таблицы (пишем только один раз)
    header = f"{' ':<42}" 
    logger.info(header)


    header = f"{'Дата':<12}{'Время':<10}{'П1':<4}{'В1':<4}" \
             f"{'Y1':<4}{'Y2':<4}{'Y4':<6}{'TEN':<6}{'T1':<6}{'T2':<6}{'T3':<8}{'T4':<6}{'T5':<6}"\
             f"{'Mode':<6}{'Филь':<6}{'V_In_Summ':<12}{'Q_Summ':<14}"
    logger.info(header)

    header = f"{' ':<12}{' ':<10}{' ':<4}{' ':<4}" \
             f"{' ':<4}{' ':<4}{' %':<6}{' %':<6}{'°C':<6}{'°C':<6}{'°C':<8}{'°C':<6}{'°C':<6}"\
             f"{' ':<6}{'тр':<6}{'м3':<12}{'Дж':<14}"
...
#################################################
# Функция для логирования HVAC5
#################################################
def log_system_state_HVAC5(system: module_model.VentilationSystem):
    current_day = datetime.now().strftime("%Y:%m:%d")
    current_time = datetime.now().strftime("%H:%M:%S")
    row = f"{current_day:<12}" \
          f"{current_time:<10}" \
          f"{system.supply_fan.status:<4}" \
          f"{system.exhaust_fan.status:<4}" \
          f"{system.supply_damper.status:<4}" \
          f"{system.exhaust_damper.status:<4}" \
          f"{str(system.bypass_damper.position):<6}" \
          f"{str(system.TEN.position):<6}" \
          f"{str(system.T1.value):<6}" \
          f"{str(system.T2.value):<6}" \
          f"{str(round(system.T3.value, 2)):<8}" \
          f"{str(system.T4.value):<6}" \
          f"{str(system.T5.value):<6}" \
          f"{str(system.mode.mode):<6}" \
          f"{system.filter_In.status:<6}" \
          f"{str(round(system.V_In_Summ,8)):<12}" \
...
def keyboard_thread():
    ...
    print("  ml — уменьшение числа людей")
    print("  mg — увеличение числа людей")
...
###########################################################################################
## Поток Modbus RTU Master.  Утилита читает регистры контроллера  и кладет их в очереди
###########################################################################################
...
############################################################################
#  Функция потока имитирующего сеть датчиков ModbusRTU T1, T2, T3, T4, T5
############################################################################
...
if __name__ == "__main__":
    ...
    # Создаём систему вентиляции
    nB1 = module_model.VentilationSystem(
        supply_fan= module_model.Fan(),
        exhaust_fan=module_model.Fan(),
        supply_damper=module_model.Damper(),
        exhaust_damper=module_model.Damper(),
        water_damper=module_model.Damper_0_100(),
        TEN = module_model.Damper_0_100(),
        bypass_damper=module_model.Damper_0_100(),
        filter_In = module_model.Filter(),
        filter_Out = module_model.Filter(),
        recuperator = module_model.Recuperator(),
        ...
        Tcool_water = module_model.Temperature(),
        S = 200,    # м^2
        m_In = 0.5,  # масса подаваемого воздуха в секунду  0.5 кг / сек
        V_In = 0.5,
        K = 3,      # (м^2 * °C) / Вт
        m = 500,     # кг
        Heat_excess = 0,   # избыток тепла в помещении кВт
        C_air = 1005,              # 1005 Дж / (кг * К)
        dt = 3,              # 3 sec  период пересчета параметров системы
        E = 0.75,              # КПД рекуператора 0.7
        V = 0,
        V_In_outside = 0,
        V_In_Summ = 0,
        Q_Summ = 0,
    ...
    while True:
        T5 = module_model.ask_float("Введите Тул: ", -5, -30, 30)
        T3 = module_model.ask_float("введите Тзоны: ", 22, 10, 30)
        nB1.V = module_model.ask_float("Объем помещения: ", 400, 50, 1000)
        nB1.m = 1.2 * nB1.V
        nB1.S = round(pow(nB1.m/1.2, 2/3) * 6, 2)
        print(f"Площадь стен [м^2] = {nB1.S}")
        nB1.Heat_excess  = module_model.ask_float("Теплоизбытки в помещении кВт: ", 0, 0, 100)
        
        nB1.m_In = module_model.ask_float("Объем подаваемого воздуха м^3 / c: ", 0.5, 0.1, 2)
        nB1.E = module_model.ask_float("Коэффициент эффективности рекуператора ε: ", 0.6, 0.3, 1)
    ...
    # Работаем с объектом
    nB1.supply_fan.status = 0
    nB1.exhaust_fan.status = 0
    nB1.supply_damper.status = 0
    nB1.exhaust_damper.status = 0
    nB1.water_damper.position = 0
    nB1.TEN.position = 0
    nB1.bypass_damper.position = 0
    ...
    nB1.T4.value = T3
    nB1.T5.value = T5
    nB1.filter_In.status = 0
    nB1.filter_Out.status = 0
    nB1.recuperator.status = 0
    nB1.mode.mode = 3
    nB1.Theat_water.value = 70
    nB1.Tcool_water.value = 7
    ...
    file_handler_table = logging.FileHandler("HVAC5_table.log", encoding="utf-8")
    ...
    file_handler_error = logging.FileHandler("HVAC5_error.log", encoding="utf-8")
    ...
    arr_main[0][0] = int(nB1.T3.value * 10)
    arr_main[1][0] = int(nB1.T3.value * 10)
    arr_main[2][0] = int(nB1.T3.value * 10)
    arr_main[3][0] = int(nB1.T3.value * 10)
    arr_main[4][0] = int(nB1.T5.value * 10)
...
##            print("interval_period:", interval_period)
            ...
            # заносим считанные и вычисленные параметры в структуру nB1
            nB1.supply_fan.status = DI[0] & 1
            nB1.exhaust_fan.status = (DI[0] & 2) >> 1
            #nB1.supply_damper.status = DO[0] & 1
            nB1.supply_damper.status = (DI[0] & 0x10) >> 4
            nB1.exhaust_damper.status = (DO[0] & 2) >> 1
            nB1.TEN.position = AI[0]
            nB1.bypass_damper.position = AI[1]
            nB1.filter_In.status = (DI[0] & 4) >> 2
            nB1.filter_Out.status = (DI[0] & 4) >> 2
            nB1.recuperator.status = (DI[0] & 8) >> 3
            nB1.mode.mode = mode[0]
            nB1.SetPoint.value = SP[0]


            # ++++ вычисляем температуры
            module_model.Recuperator_air_air_bypass_Calc_new(nB1)     # работа воздушного рекуператора
            if nB1.mode.mode == 1:
                module_model.HeatExcanger_Freon_air_Calc(nB1)       # расчет Т2 при работе канального кондиционера летом
            else:    
                module_model.HeatExchanger_TEN_air_Calc(nB1)        # работа  теплообменника ТЭН / воздух
            
            module_model.TemperatureHallCalc(nB1)               # утечка и приток энергии в помещение вычисляем температуру
            module_model.Air_In_Summ_nB1(nB1)                   # вычисления общеего объема поданого воздуха
            module_model.Q_Summ_nB(nB1)                               # вычисление затраченой энергии на
            
            # обновляем регстры температур  
            arr_main[0][0] = int(nB1.T1.value * 10)
            arr_main[1][0] = int(nB1.T2.value * 10)
            arr_main[2][0] = int(nB1.T3.value * 10)
            arr_main[3][0] = int(nB1.T4.value * 10)
            arr_main[4][0] = int(nB1.T5.value * 10)


            # Логируем текущее состояние
            log_system_state_HVAC5(nB1)
...
#---------------------------------------------------------------------------
            ...
            data = {
                "T1": nB1.T1.value,
                "T2": nB1.T2.value,
                "T3": round(nB1.T3.value, 4), #nB1.T3.value, 
                "T4": nB1.T4.value,
                "T5": nB1.T5.value,
                "T_SP": NET5[6], #nB1.SetPoint.value,
#                "co2": 0,
#                "co2_SP": 0,
                "air_damper": nB1.bypass_damper.position,
                "air_bypass": nB1.bypass_damper.position,
                "water_damper": nB1.TEN.position,
                "TEN": nB1.TEN.position,
                "Y1": nB1.supply_damper.status * 101,
                "Y2": nB1.exhaust_damper.status * 102,
                "n1": nB1.supply_fan.status * 100,
                "B1": nB1.exhaust_fan.status * 99,
                "V_In_Summ": nB1.V_In_Summ,          # Суммарный объем поданого воздуха с улицы м3
                "Q_Summ": nB1.Q_Summ,                 # Суммарная энергия закчаная через теплообменник в Дж
                "mode": nB1.mode.mode,                # режим
                "V_room": nB1.V,                      # объем помещения
                "Heat_excess": nB1.Heat_excess,       # избыток тепла
                "men": nB1.men,                       # количество людей
                "V_In_outside": nB1.V_In_outside,     # объем подаваемого с улицы воздуха
                "W": nB1.W,                            # мощьность расходуемая на изменение температуры воздуха
                "V_In_Smm": nB1.V_In_Summ,              # поданый объем воздуха
                "DI": DI[0],
                "DO": DO[0],
                "Alarms": Alarms[0],
            }
