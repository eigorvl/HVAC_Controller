...
#################################################
# @brief Функция для шапки логирования HVAC4
#################################################
def log_system_state_Head_HVAC4():
    # Заголовок таблицы (пишем только один раз)
    header = f"{' ':<42}" 
    logger.info(header)


    header = f"{'Дата':<12}{'Время':<10}{'П1':<4}" \
             f"{'Y1':<6}{'TEN':<6}{'T1':<6}{'T2':<6}{'T3':<8}{'CO2':<6}{'T5':<6}"\
             f"{'Mode':<6}{'Филь':<6}{'V_In_Summ':<12}{'Q_Summ':<14}"
    logger.info(header)

    header = f"{' ':<12}{' ':<10}{' ':<4}" \
             f"{' %':<6}{' %':<6}{'°C':<6}{'°C':<6}{'°C':<8}{'ppm':<6}{'°C':<6}"\
...
#################################################
# @brief Функция для логирования HVAC4
#################################################
def log_system_state_HVAC4(system: module_model.FancoilSystem):
    current_day = datetime.now().strftime("%Y:%m:%d")
    current_time = datetime.now().strftime("%H:%M:%S")
    row = f"{current_day:<12}" \
          f"{current_time:<10}" \
          f"{system.supply_fan.status:<4}" \
          f"{system.air_damper.position:<6}" \
          f"{str(system.water_damper.position):<6}" \
...
if __name__ == "__main__":
    ...
    # Создаём систему фанкойла
    Fancoil = module_model.FancoilSystem(
        supply_fan= module_model.Fan(),
        water_damper=module_model.Damper_0_100(),
        air_damper=module_model.Damper_0_100(),
        ...
        T_SP = 22,
        dT_SP = 1,                 # зона нечуствительности уставки к изменениям температуры
        CO2_SP = 800,
        S = 200,    # м^2
        m_In = 0.5,  # масса подаваемого воздуха в секунду  0.5 кг / сек
        V_In  = 0.6,   # объем подаваемого воздуха в секунду 0.6 м3 / сек
        K = 3,      # (м^2 * °C) / Вт
        V = 400,    # м3
        m = 500,     # кг
        ...
        W = 0,
        answer_modbus = 0,
        m_In_outside = 0,    # Количество воздуха подаваемого с улицы
        ...
        CO2_gen = 0.3 * 0.001,        # кол-во СО2 генерируемое  одним человеком в минуту 0.3 л / (мин * чел)
        V_CO2 = 0,              # объем СО2 в помещении [м3]
        dCO2_men = 0,
        CO2_ppm = 400,       # концентрация текущая СО2    
        dQ = 0,
        net1 = 0,           # отладочная информация 0x09
        net2 = 0,           #  0x0A
        net3 = 0,           #  0x0B
        net4 = 0,           #  0x0C
        net5 = 0           #  0x0D
    ...
    while True:
        ...
        Fancoil.V = module_model.ask_float("Объем помещения м3: ", 400, 50, 1000)
        Fancoil.m = 1.2 * Fancoil.V;
        Fancoil.S = round(pow(Fancoil.m/1.2, 2/3) * 6, 2)
        print(f"Площадь стен [м^2] = {Fancoil.S}")
        Fancoil.K  = module_model.ask_float("Тепловое сопротивление К [(м^2 * °C) / Вт] : ", 3, 2, 5)
        Fancoil.Heat_excess  = module_model.ask_float("Теплоизбытки в помещении кВт: ", 0, 0, 100)
    
        Fancoil.V_In = module_model.ask_float("Объем подаваемого воздуха м^3 / c: ", 0.5, 0.1, 2)
        Fancoil.m_In = 1.2 * Fancoil.V_In
        Fancoil.men = module_model.ask_float("Количество людей в помещении : ", 20, 0, 200)

        Fancoil.V_CO2 = Fancoil.m / 1.2 * 0.000400
    ...
    # Инициализируем фанкойл
    Fancoil.supply_fan.status = 0
    Fancoil.water_damper.position = 0
    Fancoil.air_damper.position = 0
    #  определяем начальные температуры
    Fancoil.T1.value = T3
    Fancoil.T2.value = T3
    Fancoil.T3.value = T3
    Fancoil.T4.value = Fancoil.T4.value = 400   #int((Fancoil.V_CO2 * 1000000)/ (Fancoil.m / 1.2))  
    Fancoil.T5.value = T5
    Fancoil.mode.mode = 3
    Fancoil.Theat_water.value = 60
    Fancoil.Tcool_water.value = 7
    ...
    file_handler_table = logging.FileHandler("HVAC4_table.log", encoding="utf-8")
    ...
    file_handler_error = logging.FileHandler("HVAC4_error.log", encoding="utf-8")
    ...
    arr_main[0][0] = int(Fancoil.T3.value * 10)
    arr_main[1][0] = int(Fancoil.T3.value * 10)
    arr_main[2][0] = int(Fancoil.T3.value * 10)
    arr_main[3][0] = int(Fancoil.T4.value)
    arr_main[4][0] = int(Fancoil.T5.value * 10)
...
##            print("interval_period:", interval_period)
            ...
            if not q_cmd.empty():
                ...
                if cmd == "t":
                    ...
                    if Fancoil.Heat_excess == 0:
                        Fancoil.Heat_excess = 0
                    else:
                        Fancoil.Heat_excess -= 1
                elif cmd == "wg":
                        Fancoil.Heat_excess += 1                                       
            ...
            Fancoil.supply_fan.status = DI[0] & 1
            Fancoil.water_damper.position = AI[0]
            Fancoil.air_damper.position = AI[1]
            Fancoil.filter_In.status = (DI[0] & 4) >> 2
            Fancoil.mode.mode = mode
            Fancoil.T_SP = SP[0]
            Fancoil.dT_SP = SP[1]
            Fancoil.CO2_SP = SP[3]
            ...
            module_model.Air_Damper_Fancoil_Temperature_Calc(Fancoil)         # Вычисление температуры подаваемого воздуха Т1
            module_model.HeatExchanger_Fancoil_TEN_air_Calc(Fancoil)          # функция вычисления температуры на выходе электронагревателя Фанкойла
            module_model.TemperatureHallCalc_Fancoil(Fancoil)                 # Изменение температуры в помещении
            
            module_model.Air_Damper_Fancoil_CO2_Calc(Fancoil)                 # расчет концентрации СО2 в помещении в ppm
            module_model.Air_In_Summ(Fancoil)                               # вычисление поданого уличного воздуха
            module_model.Q_Summ(Fancoil)                               # вычисление затраченой энергии на подогрев 
            
            # обновляем регстры температур  
            arr_main[0][0] = int(Fancoil.T1.value * 10)
            arr_main[1][0] = int(Fancoil.T2.value * 10)
            arr_main[2][0] = int(Fancoil.T3.value * 10)
            arr_main[3][0] = int(Fancoil.T4.value)
            arr_main[4][0] = int(Fancoil.T5.value * 10)

            # Логируем текущее состояние
            log_system_state_HVAC4(Fancoil)
...
#---------------------------------------------------------------------------
            ...
            data = {
                ...
                "T5": Fancoil.T5.value,
                "T_SP": Fancoil.T_SP,
                "co2": Fancoil.T4.value,
                "co2_SP": Fancoil.CO2_SP,
                "air_damper": Fancoil.air_damper.position,
                "TEN": Fancoil.water_damper.position,
                "fan": Fancoil.supply_fan.status * 100,
                "V_In_Summ": Fancoil.V_In_Summ,       # Суммарный объем поданого воздуха с улицы м3
                "Q_Summ": Fancoil.Q_Summ,          # Суммарная энергия закчаная через теплообменник в Дж  
                "air_damper": 0,
                "air_bypass": 0,
                "water_damper": Fancoil.water_damper.position,
                "Y1": 0,
                "Y2": 0,
                "n1": 0,
                "B1": 0,
                "V_In_Summ": Fancoil.V_In_Summ,          # Суммарный объем поданого воздуха с улицы м3
                "Q_Summ": Fancoil.Q_Summ,                 # Суммарная энергия закчаная через теплообменник в Дж
                "mode": Fancoil.mode.mode,                # режим
                "V_room": Fancoil.V,                      # объем помещения
                "Heat_excess": Fancoil.Heat_excess,       # избыток тепла
                "men": Fancoil.men,                       # количество людей
                "V_In_outside": Fancoil.V_In_outside,     # объем подаваемого с улицы воздуха
                "W": Fancoil.W,                            # мощьность расходуемая на изменение температуры воздуха
                "V_In_Smm": Fancoil.V_In_Summ,              # поданый объем воздуха
                "DI": DI[0],
                "DO": DO[0],
                "Alarms": Alarms[0],
            }
